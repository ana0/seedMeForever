...
const saveArchive = (req, res) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    db.get(`SELECT id, name, archiveId FROM animals WHERE id == ${req.params.id} AND archiveId IS NULL`, function(err, result) {
      ...
      db.run("INSERT INTO archive(scientificName, name, filename, hash, comments, approved) VALUES (?, ?, ?, ?, ?, ?)",
        [req.body.scientificName, req.body.humanName, req.file.filename, hash, req.body.comments, 0], function(err) {
          if (err) return reject(err)
